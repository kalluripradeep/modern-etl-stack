...
with dag:
    ...
    prune_bronze_task = PythonOperator(
        ...
        # Runs even when an ingest task failed. Verified with a purpose-built
        # probe DAG rather than assumed: a task with all_done still executes
        # when its upstream is itself upstream_failed, so the rule does reach
        # through the chain from a failed ingest task, through trigger_spark,
        # to here.
        #
        # This is cleanup, and gating it
