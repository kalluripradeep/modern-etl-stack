...
(airflow/dags/config/pipelines.yml):
  ...
  - ui/src/lib/stores.generated.ts
...
TS_HEADER = (
    '// GENERATED by scripts/generate_pipeline_assets.py '
    'from airflow/dags/config/pipelines.yml\n'
    '// Do not edit by hand: run `make generate` after changing the manifest.\n'
)


def gen_stores_ts(manifest):
    """Emit the store registry for the dashboard.

    The assistant needs the same table names and dialects as everything else,
    and it is the one consumer written in another language -- which is
    precisely why its copy of them drifted. Generating it keeps TypeScript
    honest against the same manifest without asking Node to parse YAML at
    runtime or adding a dependency to do it.
    """
    lines = [
        TS_HEADER,
        '',
    ...
    for name, store in manifest['stores'].items():
        tables = ', '.join(
            "%s: '%s'" % (t, store['table'].format(table=t))
            for t in manifest['tables']
        )
        lines += [
            '  %s: {' % name,
            "    label: '%s'," % store['label'].replace("'", "\\'"),
            "    dialect: '%s'," % store['dialect'],
            '    tables: { %s },' % tables,
            "    dateTrunc: '%s'," % store['date_trunc'],
            '  },',
        ]
    lines += [
        '};',
...
def main():
    ...
    targets = {
        ...
        REPO / 'ui' / 'src' / 'lib' / 'stores.generated.ts': gen_stores_ts(manifest),
    }
